#!/usr/bin/env python3
# dancer_master는 dancer에게 /hw/motion 토픽으로 동작 명령을 보냄.
# dancer는 dancer는 /hw/motion을 수신하면 ur5_rtde_bridge를 이용해 정해진 동작을 수행합니다. motion은, ur5_rtde_bridge가 제공하는 움직임들(tcp 절대좌표 이동, tcp 상대좌표 이동, joint 값 이동, joint 상대값 이동, 사전 정의된 위치로 이동)을 이용해서 구현하면 된다.
"""
dancer.py (TEMPLATE)
    ...
    def _on_cmd_motion(self, msg: String):
        ...
        def worker():
            try:
                ...
                if name == "test1": # 손 흔들기 모션 (wrist3 회전 후 복귀)
                    # Joint 기반 움직임 속도 조절
                ...
                elif name == "test2": # 사전 정의된 waypoint 순회 
                    # ur5_rtde_bridge.py (joint 값을 불러옴)
                    # 미리 저장해둔 pose1 -> pose2 -> pose3 -> pose2 (pose는 ur5_saved_poses.json에 정의되어 있어야 함)
                    msg = String()
                    msg.data = "go pose1"
